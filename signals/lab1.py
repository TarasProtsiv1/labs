...
class FourierApproximation:
    ...
    def compute_coefficients(self):
        x_shift = lambda x: x - (self.a + self.b) / 2.0

        a0, _ = quad(lambda x: self.f(x), self.a, self.b, limit=200)
        self.a_k[0] = a0 / (2.0 * self.L)  
        self.b_k[0] = 0.0

        for k in range(1, self.N + 1):
            ak, _ = quad(
                lambda x: self.f(x) * math.cos(k * math.pi * x_shift(x) / self.L),
                self.a,
                self.b,
                limit=200,
            )
            bk, _ = quad(
                lambda x: self.f(x) * math.sin(k * math.pi * x_shift(x) / self.L),
                self.a,
                self.b,
                limit=200,
            )
    ...
    def evaluate_fourier(self, x, N_order=None):
        ...
        x_shifted = x - (self.a + self.b) / 2.0
        val = self.a_k[0] 
        for k in range(1, N_order + 1):
            arg = k * math.pi * x_shifted / self.L
    ...
    def save_results_to_file(self, filename="fourier_results.txt"):
        ...
        with open(filename, "w", encoding="utf-8") as f:
            ...
            for k in range(1, self.N + 1):
                f.write(
                    f"a_{k} = {self.a_k[k]:.6f},  b_{k} = {self.b_k[k]:.6e}\n"
                )
